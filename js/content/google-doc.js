...
function DummyReadAloudDoc() {
  ...
  function getSource(waitDuration) {
    ...
    return new Promise(function(fulfill, reject) {
      var menu = $(".goog-menuitem-content").filter(function() {
        return $(this).text().startsWith("Kakatua TTS")
      })
      if (!menu.length) reject(new Error("'Kakatua TTS' addon not found"))
  ...
  function createAddonInstructionPopup() {
    ...
    var $popup = $("<div>")
      ...
      .data("message", "You need to install the Kakatua Google Workspace add-on to Kaktua this document.")
      .css({
        position: "absolute",
    ...
    var $text = $("<div>")
      .appendTo($popup)
      .html("Please use this menu to find and install the Kakatua Google Workspace add-on.  For instructions, see <a style='color:yellow' target='_blank' href='https://blog.readaloud.app/2021/09/google-docs-update.html'>this post</a>.")
      .css({
        backgroundColor: "#333",
        color: "#fff",
        padding: "1em",
  ...
  function createSaveInstructionPopup() {
    ...
    var $popup = $("<div>")
      .appendTo(document.body)
      .data("message", "You need to use the Kakatua Google Workspace add-on to Kakatua this document.")
      .css({
        position: "absolute",
